    ...
    override func viewDidLoad() {
        ...
        rootFlexContainer.flex.direction(.column).padding(100).define { (flex) in
            ...
            flex.addItem().direction(.column).define{ (flex) in
                ...
                rowWrapLabel.text = "Equal shrink for each item"
                flex.addItem(rowWrapLabel)
                flex.addItem().wrap(.wrap).direction(.row).define { (flex) in
                    flex.addItem(items[0]).margin(10).shrink(1)
                    flex.addItem(items[1]).margin(10).shrink(1)
                    flex.addItem(items[2]).margin(10).shrink(1)
                }
            }
            ...
            flex.addItem().direction(.column).define{ (flex) in
                ...
                flex.addItem().direction(.row).width(75).define { (flex) in
                    flex.addItem(items[3]).margin(10).shrink(1)
                    flex.addItem(items[4]).margin(10).shrink(2)
                }
            }
