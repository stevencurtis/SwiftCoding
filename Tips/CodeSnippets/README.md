...
## Storing constants
There are other ways of storing constants (using a `struct` for instance), but by using an `enum` there is no chance of accidentally instantiating your constants. Which is nice. This method also ensures that your constants are unique (it is a pure namespace). More importantly, in `Combine` Apple have used `enum` for namespaces. 
...
[https://github.com/stevencurtis/NetworkManager](https://github.com/stevencurtis/NetworkManager)
We use multiple initializers to enable a view model to access the network libarary, and can let us test
...
# Example View Model with DiffableDataSource
```swift
import Foundation
...
Example View Controller with diffabledatasource
```swift
import Foundation
import NetworkLibrary
import Combine
